...
class PPU {
	...
	public static var JSON_PROPERTIES:Array<String> = [
        // Memory
        'vramMem', 'spriteMem',
        // Counters
    ...
    public var clipToTvSize:Bool = true;
    ...
    inline public function startVBlank() {        
        // Do NMI:
	...
	inline function generateFrameContent() {
		var i:Int = 0;
		while (i < 240 * 256 * 4) {
			var color = buffer[Std.int(i / 4)];
			nes.bmp._canvas.data[i] = (color >> 16) & 255;
			nes.bmp._canvas.data[i + 1] = (color >> 8) & 255;
			nes.bmp._canvas.data[i + 2] = (color & 255);
			nes.bmp._canvas.data[i + 3] = 255;
			
			i += 4;
		}
	}
    
    inline public function endScanline() {
        switch (scanline) {
            case 19:
                // Dummy scanline.
    ...
    inline public function updateControlReg1(value:Int) {        
        triggerRendering();
    ...
    inline public function updateControlReg2(value:Int) {        
        triggerRendering();
    ...
    inline public function readStatusRegister() {        
    ...
    // horizontal offset:
    inline public function scrollWrite(value:Int) {
        triggerRendering();
    ...
    inline public function writeVRAMAddress(address:Int) {   
        if (firstWrite) {            
            regFV = (address >> 4) & 3;
            regV = (address >> 3) & 1;
            regH = (address >> 2) & 1;
    ...
    inline public function vramWrite(value:Int) {        
        triggerRendering();
        cntsToAddress();
        regsToAddress();
    ...
    // into Sprite RAM.
    inline public function sramDMA(value:Int) {
    ...
    inline public function regsFromAddress() {        
    ...
    // Updates the scroll registers from a new VRAM address.
    inline function cntsFromAddress() {        
    ...
    inline function incTileCounter(count:Int) { 
		var i:Int = count;
		while(i != 0) {
            cntHT++;
            if (cntHT == 32) {
    ...
    // mirroring/mapping of address ranges.
    inline function mirroredWrite(address:Int, value:Int) {
    ...
    inline function renderFramePartially(startScan:Int, scanCount:Int) {
        if (f_spVisibility == 1) {
            ...
            for (tile in 0...32) {                
                if (scan >= 0) {                
                    ...
                    if (validTileData) {
                        // Get data from array:
                        t = scantile[tile];
						if (t == null) {	// fix crashes on some mapper 4 games
							continue;
						}
                        tpix = t.pix;
                        att = attrib[tile];						
    ...
    inline function updatePalettes() {        
        for (i in 0...16) {
            if (f_dispType == 0) {
    ...
    inline function patternWrite(address:Int, value:Int) {
    ...
    public function toJSON():Dynamic {
        ...
        state.nameTable = [];
        for (i in 0...nameTable.length) {
            state.nameTable[i] = nameTable[i].toJSON();
        }
        
        state.ptTile = [];
        for (i in 0...ptTile.length) {
    ...
    public function fromJSON(state:Dynamic) {        
        Utils.fromJSON(this, state);
        
        for (i in 0...nameTable.length) {
            nameTable[i].fromJSON(state.nameTable[i]);
        }
