...
class MapperDefault {
    ...
    inline function regWrite(address:Int, value:Int) {
        switch (address) {
            case 0x2000:
                // PPU Control register 1
                nes.cpu.mem[address] = value;
    ...
    inline function loadRomBank(bank:Int, address:Int) {
    ...
    inline function loadVromBank(bank:Int, address:Int) {
        if (nes.rom.vromCount != 0) {
			...
			Vector.blit(nes.rom.vrom[bank % nes.rom.vromCount], 0, nes.ppu.vramMem, address, 4096);
			...
			Vector.blit(vromTile, 0, nes.ppu.ptTile, address >> 4, 256);
        }        
    }
    ...
    inline function load1kVromBank(bank1k:Int, address:Int) {
        if (nes.rom.vromCount != 0) {
			...
			Vector.blit(nes.rom.vrom[bank4k], 0, nes.ppu.vramMem, bankoffset, 1024);
    ...
    inline function load2kVromBank(bank2k:Int, address:Int) {
        if (nes.rom.vromCount != 0) {
			...
			Vector.blit(nes.rom.vrom[bank4k], bankoffset, nes.ppu.vramMem, address, 2048);
    ...
    inline function load8kRomBank(bank8k:Int, address:Int) {
		...
		Vector.blit(nes.rom.rom[bank16k], offset, nes.cpu.mem, address, 8192);
    }
